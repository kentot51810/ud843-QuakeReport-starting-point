...
/**
 * Created by Kenn Nacion on 014, 14 Aug 2017.
 */
...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            // Create an object for feature directory.
            // This directory will serve as the root for the array.
            JSONArray feature = root.getJSONArray("features");

            // Loop through the Array Until we all get the elements
            for (int index=0; index < feature.length(); index++){
                JSONObject rootArray = feature.getJSONObject(index);
                JSONObject properties = rootArray.getJSONObject("properties");
                ...
                //Convert UNIX EPOCH TIME MILLISECONDS to an Actual Readable Date
                Date date = new Date(time);
                SimpleDateFormat dateFormatter = new SimpleDateFormat("MMM DD, yyyy");
                SimpleDateFormat hourFormatter = new SimpleDateFormat("h:mm a");
                String convertedDate = dateFormatter.format(date);
                String convertedTime = hourFormatter.format(date);

                earthquakes.add(new Earthquake(magnitude, place, convertedDate, convertedTime, url));
                Log.e(TAG, earthquakes.toString());
            }
